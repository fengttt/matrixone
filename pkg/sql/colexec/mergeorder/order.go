...
func (ctr *container) mergeSort2(bat2 *batch.Batch, proc *process.Process) error {
	...
	// XXX
	//
	// Moved setting up vector in cmp from before the UnionBatch loop
	// to after.  vector.UnionBatch may extend the bat1, which may call
	// realloc therefore, if this loop is called before UnionBatch, the
	// vector can be garbaged.
