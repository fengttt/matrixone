	...
	if len(exprs) == 1 {
		...
		if strings.Contains(optstr, "{") {
			...
			for idx, c := range optstr {
				switch c {
				case 'b':
					opts.Cols = append(opts.Cols, ParseJsonlOptionsCol{Name: fmt.Sprintf("col%d", idx), Type: "boolean"})
				case 'i':
					opts.Cols = append(opts.Cols, ParseJsonlOptionsCol{Name: fmt.Sprintf("col%d", idx), Type: "int32"})
				case 'I':
					opts.Cols = append(opts.Cols, ParseJsonlOptionsCol{Name: fmt.Sprintf("col%d", idx), Type: "int32"})
				case 'f':
					opts.Cols = append(opts.Cols, ParseJsonlOptionsCol{Name: fmt.Sprintf("col%d", idx), Type: "float32"})
				case 'F':
					opts.Cols = append(opts.Cols, ParseJsonlOptionsCol{Name: fmt.Sprintf("col%d", idx), Type: "float64"})
				case 's':
					opts.Cols = append(opts.Cols, ParseJsonlOptionsCol{Name: fmt.Sprintf("col%d", idx), Type: "string"})
				case 't':
					opts.Cols = append(opts.Cols, ParseJsonlOptionsCol{Name: fmt.Sprintf("col%d", idx), Type: "timestamp"})
				default:
