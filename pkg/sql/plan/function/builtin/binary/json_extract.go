...
func JsonExtract(vectors []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	result := json_extract.JsonExtract(json, path, nil)
	resultVec := vector.New(resultType)
	if err := vector.AppendBytes(resultVec, result, proc.Mp()); err != nil {
		return nil, err
	}
	logutil.Infof("JsonExtract: resultVector=%s,type:%T", resultVec, resultVec.Col)
