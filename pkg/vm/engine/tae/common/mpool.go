...
func InitTAEMPool() {
	onceBody := func() {
		...
		if DefaultAllocator, err = mpool.NewMPool("tae_default", 0); err != nil {
			panic(err)
		}

		mpool.DeleteMPool(MutMemAllocator)
		if MutMemAllocator, err = mpool.NewMPool("tae_immutable", 0); err != nil {
			panic(err)
		}

		mpool.DeleteMPool(CacheAllocator)
		if CacheAllocator, err = mpool.NewMPool("tae_cache", 0); err != nil {
			panic(err)
		}

		mpool.DeleteMPool(LogAllocator)
		if LogAllocator, err = mpool.NewMPool("tae_log", 0); err != nil {
			panic(err)
		}
	}
	once.Do(onceBody)
